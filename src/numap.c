...
{
    printf("func=%s, info->si_code=%d\n", __func__, info->si_code);
    if (info->si_code == POLL_HUP)
    {
        numap_samping_print_test();
    }
    // numap_samping_print_test();
    // ioctl(info->si_fd, PERF_EVENT_IOC_REFRESH, 1000);
    // if (info->si_code == POLL_HUP)
    // {
...
int numap_sampling_init_measure(struct numap_sampling_measure *measure, int nb_threads, int sampling_rate, int mmap_pages_count)
{
    ...
    measure->nb_refresh = 10; // default refresh
...
static int __numap_sampling_resume(struct numap_sampling_measure *measure)
{
    ...
    for (thread = 0; thread < measure->nb_threads; thread++)
    {
        ...
        fcntl(measure->fd_per_tid[thread], F_SETSIG, SIGUSR1);
        fcntl(measure->fd_per_tid[thread], F_SETOWN, getpid());
        ioctl(measure->fd_per_tid[thread], PERF_EVENT_IOC_REFRESH, 10);
...
int __numap_sampling_start(struct numap_sampling_measure *measure, struct perf_event_attr *pe_attr)
{
    ...
    for (thread = 0; thread < measure->nb_threads; thread++)
    {
        ...
        measure->fd_per_tid[thread] = perf_event_open(pe_attr, -1, measure->cpu + 32,
                                                      -1, 0);
...
int numap_sampling_read_start_generic(struct numap_sampling_measure *measure, uint64_t sample_type)
{
    ...
    curr_err = pfm_get_os_event_encoding("MEM_UOPS_RETIRED:ALL_LOADS", PFM_PLM0 | PFM_PLM3, PFM_OS_PERF_EVENT, &arg);
    if (curr_err != PFM_SUCCESS)
    {
        return ERROR_PFM;
    }
...
{
    return numap_sampling_read_start_generic(measure, PERF_SAMPLE_TID | PERF_SAMPLE_ADDR | PERF_SAMPLE_TIME | PERF_SAMPLE_DATA_SRC);
}
